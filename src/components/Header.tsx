...
export default function Header() {
    ...
    return (
        <header className="sticky top-0 z-50 w-full border-b border-[#2A2A33] bg-background-dark/90 backdrop-blur-md">
            <div className="w-full h-20 flex items-center justify-between px-6 lg:px-[120px]">
                <Link to="/" className="flex items-center gap-4 group">
                    {/*<div className="relative flex size-12 items-center justify-center bg-primary/5 border border-primary/20 group-hover:border-primary/50 transition-all duration-500">
                        <div className="absolute top-0 left-0 size-2 border-l border-t border-primary group-hover:size-full transition-all duration-500"></div>
                        <div className="absolute bottom-0 right-0 size-2 border-r border-b border-primary group-hover:size-full transition-all duration-500"></div>
                        <span className="material-symbols-outlined text-[28px] text-primary group-hover:scale-110 transition-transform duration-500">terminal</span>
                    </div>*/}
                    <div className="flex flex-col leading-none">
                        ...
                        {/*<span className="text-[0.6rem] text-text-gray/60 tracking-[0.3em] uppercase -mt-0.5">Designer Gráfico</span>*/}
                    </div>
                </Link>
                ...
                <div className="hidden md:flex items-center gap-4">
                    <div className="h-8 w-px bg-[#2A2A33] mx-2"></div>
                    <a
                ...
                <AnimatePresence>
                    {isMenuOpen && (
                        <motion.div
                            ...
                            exit={{ opacity: 0, y: -20 }}
                            className="absolute top-0 left-0 w-full h-screen bg-background-dark/95 backdrop-blur-xl flex flex-col items-center justify-center gap-8 md:hidden z-40"
                        >
                            <div className="flex flex-col items-center gap-8">
                                {navLinks.map((link) => (
                                    <a
                                        ...
                                        className="text-2xl font-display font-medium text-white hover:text-primary transition-colors uppercase tracking-widest"
                                    >
                                        {link.name}
                                    </a>
                                ))}
